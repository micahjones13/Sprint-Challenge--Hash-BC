...
def valid_proof(last_hash, proof):
    ...
    lhh = f'{last_hash}'.encode()
    lhg = hashlib.sha256(lhh).hexdigest()

    # encode_string = f'{str_last_hash}'.encode()
    # digest_str = hashlib.sha256(encode_string).hexdigest()

    return guess_hash[:6] == lhg[-6:]
